...
impl FileType {
    ...
    /// Check if this is an image preview file type.
...
impl Tab {
    ...
    /// Create a read-only image preview tab from a file path.
...
impl AppState {
    ...
    pub fn open_file_with_focus(
        ...
        if file_type.is_image() {
            let auto_save_default = self.settings.auto_save_enabled_default;
            let tab = Tab::with_image_file(self.next_tab_id, path.clone(), auto_save_default);
            self.next_tab_id += 1;
            self.tabs.push(tab);
            ...
            if focus {
                ...
                info!("Opened image preview: {}", path.display());
            } else {
                info!("Opened image preview in background: {}", path.display());
            }
...
mod tests {
    ...
    #[test]
    fn test_open_image_file_as_preview() {
        use std::io::Write;

        let temp_dir = std::env::temp_dir();
        let temp_file = temp_dir.join("ferrite_test_image_preview.png");
        std::fs::File::create(&temp_file)
            .unwrap()
            .write_all(include_bytes!("../assets/icons/icon_256.png"))
            .unwrap();
